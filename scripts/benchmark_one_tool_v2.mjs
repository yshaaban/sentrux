...
const outputPath =
  process.env.OUTPUT_PATH ?? path.join(repoRoot, 'docs/v2/examples/one-tool-benchmark.json');
...
const publicPathReplacements = [
  [compareToPath, '<sentrux-root>/docs/v2/examples/one-tool-benchmark.json'],
  [sentruxBin, '<sentrux-root>/target/debug/sentrux'],
  [oneToolRoot, '<one-tool-root>'],
...
async function runBenchmarkSession(workRoot, homeOverride, fixedNowEpoch) {
  return runBenchmarkSessionPhases({
    ...
    warmOperations: [
      { tool: 'project_shape' },
      { tool: 'findings', args: findingsArgs },
      { tool: 'agent_brief', args: onboardingBriefArgs },
    ],
...
async function main() {
  ...
  assertPathExists(oneToolRoot, 'one-tool repo');
  ...
  const { freshnessMetadata, samples } = await collectFrozenBenchmarkSamples({
    sourceRoot: oneToolRoot,
    cloneLabel: 'one-tool-benchmark-source',
    analysisMode,
    ...
    buildFreshnessMetadata: function buildFreshness(frozenSourceRoot) {
      return buildRepoFreshnessMetadata({
        repoRoot: resolveFreshnessRepoRoot(analysisMode, frozenSourceRoot, oneToolRoot),
    ...
    runBenchmarkSession,
    sampleLabel: 'one-tool-benchmark-sample',
  });
  ...
  console.log(
    `Wrote one-tool v2 benchmark to ${outputPath} using ${aggregate.sample_count} sample(s) with median aggregation.`,
  );
