...
test('loadArtifactInput reads codex batch bundles from per-task artifacts', async function () {
  ...
  try {
    ...
    await writeJson(batchPath, {
      repo_root: '/tmp/parallel-code',
      results: [
        { output_dir: taskOneDir },
        { output_dir: taskTwoDir },
      ],
    });
...
test('parseArgs rejects multiple artifact inputs', function () {
  assert.throws(() => {
    parseArgs(['node', 'script', '--bundle', 'a.json', '--codex-batch', 'b.json']);
  }, /Provide only one of --bundle, --codex-batch, or --replay-batch/);
});
