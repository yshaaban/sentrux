...
use self::call_distribution::distribute_calls_to_functions;
use self::captures::{
    classify_captures, process_class_def, process_func_def, process_import, ImportContext,
    MatchKind, ParseContext,
};
use self::same_file_refs::annotate_same_file_reference_counts;
...
impl ExtractionState {
    ...
    fn into_structural_analysis(mut self, tree: &Tree, content: &[u8]) -> StructuralAnalysis {
        let module_calls = distribute_calls_to_functions(&self.calls_raw, &mut self.functions);
        annotate_same_file_reference_counts(tree, content, &mut self.functions);
        StructuralAnalysis {
...
pub(crate) fn extract_with_queries(
    ...
    state.into_structural_analysis(tree, content)
}
