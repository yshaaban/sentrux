...
import { expressionPath, isAssignmentOperator, lineOfNode, literalExpressionText, propertyNameText, unwrapObjectLiteralExpression, } from "./analysis-utils.js";
import { closedDomainInfoForExpression, closedDomainInfoForTypeNode } from "./analysis-closed-domains.js";
export function collectTransitionSite(context, node) {
...
function collectSwitchTransitionSites(context, node) {
    ...
    for (const clause of node.caseBlock.clauses) {
        ...
        groupSites.push({
            ...
            source_variant: sourceVariant,
            target_variants: collectTransitionTargetVariants(clause.statements, domainInfo.variants),
    ...
    if (groupSites.some((site) => site.target_variants.length > 0) ||
        (hasPotentialIntent && groupSites.length > 0)) {
        context.transitionSites.push(...groupSites);
    }
}
function collectIfTransitionSites(context, node) {
    const groupSites = [];
    let current = node;
    let domainInfo = null;
    ...
    while (current) {
        ...
        groupSites.push(buildIfBranchTransitionSite(context, current, match, groupLine, thenStatements));
        hasPotentialIntent =
            hasPotentialIntent ||
        ...
        if (domainInfo) {
            hasPotentialIntent = appendIfElseTransitionSites(context, current, domainInfo, groupLine, matchedVariants, groupSites, hasPotentialIntent);
        }
        current = undefined;
    }
    if (groupSites.some((site) => site.target_variants.length > 0) ||
...
function matchesIfTransitionGroup(domainInfo, subjectPath, match) {
    if (!domainInfo) {
        return true;
    }
    return (subjectPath === match.subjectPath &&
...
function buildIfBranchTransitionSite(context, current, match, groupLine, thenStatements) {
    return {
        path: context.relativePath,
        ...
        target_variants: collectTransitionTargetVariants(thenStatements, match.domainInfo.variants),
        line: lineOfNode(context.sourceFile, current.expression),
    };
}
function appendIfElseTransitionSites(context, current, domainInfo, groupLine, matchedVariants, groupSites, hasPotentialIntent) {
    const elseStatements = statementsOf(current.elseStatement);
    const remainingVariants = domainInfo.variants.filter((variant) => !matchedVariants.has(variant));
    const elseTargets = collectTransitionTargetVariants(elseStatements, domainInfo.variants);
    const nextHasPotentialIntent = hasPotentialIntent || hasPotentialTransitionIntent(context, elseStatements, domainInfo);
...
function ifConditionVariant(context, expression, expectedSubjectPath, expectedDomainInfo) {
    ...
    if (leftPath) {
        const domainInfo = expectedSubjectPath === leftPath && expectedDomainInfo
            ? expectedDomainInfo
            : closedDomainInfoForExpression(context, current.left);
        const sourceVariant = domainInfo
            ? sourceVariantForCondition(domainInfo.variants, transitionVariantText(current.right, domainInfo.variants), current.operatorToken.kind)
            : null;
    ...
    if (rightPath) {
        const domainInfo = expectedSubjectPath === rightPath && expectedDomainInfo
            ? expectedDomainInfo
            : closedDomainInfoForExpression(context, current.right);
        const sourceVariant = domainInfo
            ? sourceVariantForCondition(domainInfo.variants, transitionVariantText(current.left, domainInfo.variants), current.operatorToken.kind)
            : null;
        if (domainInfo && sourceVariant) {
            return {
                domainInfo,
...
function isTransitionComparisonOperator(kind) {
...
function hasPotentialTransitionIntent(context, statements, domainInfo) {
    function domainTyped(expression) {
...
function unwrapTransitionExpression(expression) {
...
function transitionVariantText(expression, allowedVariants) {
    const current = unwrapTransitionExpression(expression);
