...
- `task_git_status` and `server_state_bootstrap` both produce structurally meaningful concept reports
...
- authoritative state
- still has more than one write path
- high leverage for authority purity
...
- clone-drift candidates in diff parsing and shared escaping helpers
- `task_git_status` has multiple durable write paths
- presentation consumers bypass canonical accessors in specific files
- app modules bypass the intended store barrel or mutate store internals directly
- bootstrap category changes require updates across registry and runtime adapters
- `task-command-lease.ts` is a concentrated coordination hotspot
...
- [x] verify authority purity on `task_git_status`
  - current result: the concept now resolves to the 2 production writer files, and the polling path remains the high-severity forbidden writer
...
- [ ] verify store-barrel bypass and direct `setStore` findings in `src/app`
