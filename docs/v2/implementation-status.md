...
| Phase 1: Harden the intervention-grade signal set | In progress | clone drift, authority/access, obligation completeness, trust-tiered findings, cohort lead priorities, and evidence-aware default-lane compression are shipping | broader cross-repo proof and more live false-positive review are still needed before the narrowed lane can be treated as fully stable |
| Phase 2: Expand the semantic obligation graph | In progress | closed-domain and contract-driven obligation expansion now carry surface-aware DTO, registry, public-API, config, command-status, and test/doc repair guidance | richer contract families, finer changed-symbol precision, and stronger sibling-surface linking remain incomplete |
| Phase 3: Add bounded LLM adjudication | In progress | the eval/runtime surfaces now have a structured-evidence adjudication scaffold with MiniMax M2.7-oriented prompts, audit logging, and explicit advisory-only guardrails | the scaffold is not yet connected to a production reranking loop, and signal-matched treatment evidence still needs to justify any live ranking effect |
...
| Phase 5: Build the treatment-vs-baseline evidence program | In progress | scorecards, remediation evals, session telemetry, diff replay, signal-matched experiment comparisons, and default-rollout qualification now exist in the evidence artifacts | controlled paired baseline/treatment runs are not yet the decisive promotion evidence |
| Phase 6: Product surface compression | In progress | `check`, `agent_brief`, trust tiers, repair-packet tooling, and lane-aware lead selection now enforce a 1-3 slot default lane with structural pressure suppressed unless it is patch-worsened and repairable | the remaining gap is proving that the same compression quality holds across more public repos and live usage, not wiring the cap itself |
...
- scorecard, session-corpus, and evidence-review artifacts now also carry reviewer acceptance/disagreement telemetry plus signal-matched default-rollout readiness at the per-signal and program-summary levels
- bounded adjudication now has a conservative scaffold in the eval/runtime surfaces: structured evidence bundles, schema-checked provider responses, cited-evidence auditing, advisory-only decision records, and phase/progress rollups for MiniMax M2.7-style evaluation
